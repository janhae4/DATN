...
      const newConversation = await ApiService.createDirectChat(selectedUser.id);
      
...
              <div
                className="bg-indigo-100 text-indigo-700 px-3 py-2 rounded-full flex items-center justify-between gap-2 text-sm"
              >
                <span>{selectedUser.name} ({selectedUser.providerId || selectedUser.email})</span>
                <button
                  type="button"
...
                
                {/* Kết quả tìm kiếm */}
                {isSearching && <p className="text-gray-500 text-sm mt-1">Đang tìm...</p>}
