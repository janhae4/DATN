...
import { 
  ...
  FileText, FileImage, FileSpreadsheet, FileCode, FileArchive, 
  FileAudio, FileVideo, Presentation, File as FileIcon,
  Info,
  MoreHorizontal
} from "lucide-react"
import {
...
// Cập nhật hàm getFileTheme để trả về cả Component Icon
const getFileTheme = (fileType: string, name: string = '') => {
  ...
  if (type.includes('presentation') || type.includes('powerpoint')) return base.presentation;
  ...
  if (type.includes('code') || type.includes('text') || type.includes('javascript') || type.includes('typescript')) return base.code;
  if (type.includes('document') || name.match(/\.(docx?|doc|odt|rtf)$/)) return base.document;
...
export const FileCard = ({ file, onPreview, onDownload, onDelete }: FileCardProps) => {
    ...
    <div className="group relative flex flex-col bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-xl transition-colors duration-200 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 hover:border-zinc-300 dark:hover:border-zinc-700">
      ...
      <div className="absolute top-3 right-3 z-10">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[160px] rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#09090b] shadow-xl shadow-zinc-200/20 dark:shadow-none">
            <DropdownMenuItem onClick={handlePreview} className="text-xs py-2 cursor-pointer gap-2">
              <ExternalLink className="h-3.5 w-3.5" /> Xem trước
            </DropdownMenuItem>
            <DropdownMenuItem onClick={handleDownload} className="text-xs py-2 cursor-pointer gap-2">
            ...
            <DropdownMenuItem onClick={handleDelete} className="text-xs py-2 cursor-pointer gap-2 text-red-600 focus:text-red-600 focus:bg-red-50 dark:focus:bg-red-900/20">
      ...
      >
        ...
        <div className="flex items-start justify-between">
          <div className={`w-10 h-10 flex items-center justify-center rounded-lg border ${theme.bg} ${theme.border}`}>
        ...
        <div className="space-y-1">
          <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate pr-6 leading-tight" title={file.fileName}>
            {file.fileName}
          </h3>
          ...
          <div className="flex justify-between ">
            <div className="flex items-center gap-2 text-[11px] text-zinc-500 dark:text-zinc-400 font-medium uppercase tabular-nums tracking-tight">
                <span>{new Date(file.uploadedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
