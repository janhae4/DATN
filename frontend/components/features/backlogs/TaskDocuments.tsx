...
export function TaskDocuments({
  ...
  const { data: attachedFiles = [], isLoading: isLoadingAttached } = useQuery<
    IFile[]
  >({
  ...
  const { data: allFilesData, isLoading: isLoadingAllFiles } = useQuery({
    ...
    queryFn: () => fileService.getFiles(projectId, teamId, 1, 100), // Get first 100 files
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { uploadUrl, fileId } = await fileService.initiateUpload(
        { fileName: file.name, fileType: file.type, parentId: null },
        projectId,
      );
  ...
  const handlePreview = async (file: any) => {
    try {
      const { viewUrl } = await fileService.getPreviewUrl(
        file._id || file.id,
        projectId,
      );
