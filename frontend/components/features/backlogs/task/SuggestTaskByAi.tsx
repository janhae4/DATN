...
export function SuggestTaskByAi({
  ...
  const handleAiSuggest = async () => {
    ...
    try {
            ...
            try {
              ...
              if (parsed.type === "task") {
                ...
                if (parsed.retryAfter) {
                  const waitTime = Math.ceil(parsed.retryAfter);
                  toast.error(`Service limit reached. Please wait ${waitTime}s before retrying.`, {
                    duration: 6000,
                    icon: "⏳",
                  });
                } else {
                  toast.error(parsed.message || "An unexpected error occurred during task suggestion.");
                }
              }
            } catch (e) {
      ...
      console.error(error);
      const errorMessage = error instanceof Error ? error.message : "AI Architect is offline.";
  ...
  const handleImport = async (tasks: SuggestedTask[]) => {
    ...
    if (onSave) {
      onSave(normalizedTasks, summary, selectedSprintId || undefined);
    }
    setOpen(false);
  };
