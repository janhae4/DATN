                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <button className={cn(
                                    ...
                                    {task.skillNames?.[0] || "Select Skill"}
                                </button>
                            </PopoverTrigger>
                            <PopoverContent className="w-[200px] p-0 shadow-xl bg-white dark:bg-[#1A1A1C] border-zinc-200 dark:border-white/10" align="start">
                                <Command shouldFilter={false} className="bg-transparent text-zinc-900 dark:text-white/90">
                                    ...
                                    <CommandList>
                                        {skillSearch && (
                                            <CommandGroup><CommandItem onSelect={() => { onUpdate(task.id, { skillNames: [skillSearch] }); setSkillSearch(""); }} className="text-xs cursor-pointer aria-selected:bg-zinc-100 dark:aria-selected:bg-white/10"><Plus className="mr-2 h-3 w-3" />Create "{skillSearch}"</CommandItem></CommandGroup>
                                        )}
                                        <CommandGroup heading="Popular">
                                            {["React", "Node.js", "TypeScript", "Python", "Design", "DevOps"].map(s => (
                                                <CommandItem key={s} onSelect={() => { onUpdate(task.id, { skillNames: [s] }); setSkillSearch(""); }} className="text-xs cursor-pointer aria-selected:bg-zinc-100 dark:aria-selected:bg-white/10">
                                                    {s} {task.skillNames?.[0] === s && <Check className="ml-auto h-3 w-3" />}
                                                </CommandItem>
                                            ))}
                                        </CommandGroup>
                                    </CommandList>
