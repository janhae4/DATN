...
import { format, subMonths, parseISO, formatDistanceToNow, differenceInCalendarMonths } from "date-fns";
import {
  FolderPlus,
  UserPlus,
  Activity,
...
const COLORS = {
  ...
  chart1: "#3b82f6", // Blue
  chart2: "#ec4899", // Pink
  pie: ["#3b82f6", "#10b981", "#f59e0b", "#ef4444"] // Blue, Emerald, Amber, Red
};
  ...
  const { growthData, monthlyPulse } = useMemo(() => {
    ...
    const data = last6Months.map((date) => {
      ...
      const projectsCount = projects.filter(p => {
        const createDate = p.createdAt ? parseISO(p.createdAt) : new Date();
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
      ...
      <div className="col-span-4 space-y-6">
        ...
        {/* GROWTH CHART CARD */}
        <Card id="analytics-growth-chart" className="shadow-sm border-muted/60">
          <CardHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {/* ROLE DONUT CHART */}
          <Card id="analytics-role-distribution" className="flex flex-col shadow-sm border-muted/60">
          ...
          {/* PULSE SUMMARY CARD */}
          <Card id="analytics-monthly-pulse" className="flex flex-col justify-between shadow-sm border-muted/60 bg-gradient-to-br from-primary/5 via-transparent to-transparent">
            <CardHeader>
      ...
      <Card id="analytics-activity-log" className="col-span-3 flex flex-col h-full shadow-sm border-muted/60">
