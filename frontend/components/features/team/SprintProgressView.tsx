...
export function SprintProgressView({
  ...
  const stats = useMemo(() => {
    if (!tasks.length || !lists.length)
      return {
        ...
        total: 0,
        progress: 0,
        categoryMap: {} as Record<string, ListCategoryEnum>,
      };
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter(
      (t) => stats.categoryMap?.[t.listId] === filterCategory
    );
            ...
            {filteredTasks.map((task) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <Checkbox
                    checked={
                      stats.categoryMap?.[task.listId] === ListCategoryEnum.DONE
                    }
                    onCheckedChange={(checked) => {
                      const targetCategory = checked
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 overflow-hidden">
                      <PriorityPicker
                        ...
                        onPriorityChange={(p) =>
                          updateTask(task.id, { priority: p || undefined })
                        }
                      />
                      ...
                      <div className="flex items-center gap-1.5">
                        {task.taskLabels?.slice(0, 3).map((l: any) => (
                          <span
                            key={l.id}
                            className="text-[10px] font-semibold text-muted-foreground/70 bg-muted/60 px-2 py-0.5 rounded-full border border-border/10"
                          >
                            {l.name}
                          </span>
                        ))}
...
function StatCard({ label, value, active, onClick, color }: StatCardProps) {
  const themes = {
    indigo:
      "indigo-500/50 indigo-50/50 indigo-500/5 indigo-600 indigo-400 indigo-600/50 indigo-500/10",
    emerald:
      "emerald-500/50 emerald-50/50 emerald-500/5 emerald-600 emerald-400 emerald-600/50 emerald-500/10",
    orange:
      "orange-500/50 orange-50/50 orange-500/5 orange-600 orange-400 orange-600/50 orange-500/10",
    slate:
      "slate-500/50 slate-50/50 slate-500/5 slate-600 slate-400 slate-600/50 slate-500/10",
  };

  const getThemeClasses = (c: keyof typeof themes) => {
    const [hover, bg, darkBg, text, darkText, activeBorder, activeRing] =
      themes[c].split(" ");
    return {
