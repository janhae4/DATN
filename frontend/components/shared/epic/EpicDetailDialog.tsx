...
export function EpicDetailDialog({
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    const todoList =
      lists?.find((l) => l.category === ListCategoryEnum.TODO) || lists?.[0];
    if (!todoList) {
            ...
            <div className="flex items-start gap-5 flex-1 w-full">
              <Popover>
                <PopoverTrigger asChild>
                  <button
                    className={cn(
                      "mt-2 h-6 w-6 rounded-full shrink-0 ring-4 ring-offset-2 transition-all hover:scale-105 focus:outline-none cursor-pointer"
                    )}
                    ...
                    <SelectTrigger className="h-8 w-auto min-w-[120px] text-xs border-0 bg-slate-100 hover:bg-slate-200 focus:ring-0 rounded-full px-3 transition-colors">
                      <div className="flex items-center gap-2">
                        {statusMap[status]?.icon &&
                          React.createElement(statusMap[status].icon, {
                            className: cn(
                              "h-3.5 w-3.5",
                    ...
                    <SelectTrigger className="h-8 w-auto min-w-[110px] text-xs border-0 bg-slate-100 hover:bg-slate-200 focus:ring-0 rounded-full px-3 transition-colors">
                      <div className="flex items-center gap-2">
                        {priorityMap[priority]?.icon &&
                          React.createElement(priorityMap[priority].icon, {
                            className: cn(
                              "h-3.5 w-3.5",
                  ...
                  {epicTasks.length > 0 ? (
                    <div className="grid gap-2">
                      {epicTasks.map((task) => (
                        <div
                          key={task.id}
                          className="group flex items-center gap-3 p-3 text-sm bg-white border rounded-lg shadow-sm hover:border-primary/50 transition-colors"
                        >
                          <div
                            className={cn(
                          ...
                          />
                          <span className="font-medium text-slate-700 leading-snug flex-1 truncate">
                            {task.title}
                          </span>
                        </div>
                      ))}
