...
export class ProjectsService {
  ...
  async create(createProjectDto: CreateProjectDto) {
    ...
    if (createProjectDto.ownerId) {
      try {
        ownerProfile = await this.amqp.request<any>({
          exchange: USER_EXCHANGE,
          routingKey: USER_PATTERNS.FIND_ONE,
          payload: createProjectDto.ownerId,
        });
      } catch (error) {
        console.warn(`Failed to fetch owner profile for project creation: ${error.message}`);
      }
    }
    ...
    this.amqp.publish(EVENTS_EXCHANGE, PROJECT_EVENTS.PROJECT_CREATED, {
