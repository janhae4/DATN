...
export class VideoChatController {
  ...
  handleJoinRoom(@Payload() data: { clientId: string; roomId: string }) {
    console.log(`VideoChat: Client ${data.clientId} joined room ${data.roomId}`);
  ...
  handleOffer(@Payload() data: { clientId: string; sdp: any; roomId: string; targetId: string }) {
    console.log(`VideoChat: Offer from ${data.clientId} to ${data.targetId} in room ${data.roomId}`);
    // TODO: Add business logic for offer handling if needed
    return { success: true, type: 'offer', from: data.clientId, to: data.targetId };
  }

  @MessagePattern('answer')
  handleAnswer(@Payload() data: { clientId: string; sdp: any; roomId: string; targetId: string }) {
    console.log(`VideoChat: Answer from ${data.clientId} to ${data.targetId} in room ${data.roomId}`);
    // TODO: Add business logic for answer handling if needed
    return { success: true, type: 'answer', from: data.clientId, to: data.targetId };
  }

  @MessagePattern('ice-candidate')
  handleIceCandidate(@Payload() data: { clientId: string; candidate: any; roomId: string; targetId: string }) {
    console.log(`VideoChat: ICE candidate from ${data.clientId} to ${data.targetId} in room ${data.roomId}`);
    // TODO: Add business logic for ICE candidate handling if needed
    return { success: true, type: 'ice-candidate', from: data.clientId, to: data.targetId };
  }
