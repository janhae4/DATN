...
import { AnswerDto, IceCandidateDto, OfferDto } from './dto/webtrc.dto';
  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleDisconnect(client: Socket) {
    ...
    for (const roomId in this.rooms) {
      ...
      if (userIndex !== -1) {
        ...
        console.log(
          `🚪 User ${client.id} left room ${roomId}. Remaining members: ${this.rooms[roomId].length > 0 ? this.rooms[roomId].join(', ') : 'None'}`,
        );
        break;
      }
    }
  ...
  handleOffer(
    ...
    client
      .to(data.targetId)
      .emit('offer', { sdp: data.sdp, senderId: client.id });
  }
  ...
  handleAnswer(
    ...
    client
      .to(data.targetId)
      .emit('answer', { sdp: data.sdp, senderId: client.id });
  }
